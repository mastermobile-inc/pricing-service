...
delivery_control:
  risk: high
  phase_records:
    - docs/phases/customer-price-type-search-task-2169-20260912.yml
...
ОТМЕНЕНО (2026-08-12): глобальный поиск по всем клиентам в v1 не нужен и не
входит в scope. Руководителю нужен единый поиск по разрешённому портфелю, в том
числе по клиентам вне его рабочих очередей. Один и тот же detail открывается из
строки списка и из результата поиска: это одна каноническая карточка, а не два
разных представления клиента.

Для дефекта поиска из задачи №2169 действует порядок: сначала исправление,
адресные тесты и проверка в production под ролью руководителя на клиенте
`РБ002793`; только после подтверждённого результата публикуется ответ
пользователю в чате задачи. До production-readback текст ответа остаётся
черновиком.
