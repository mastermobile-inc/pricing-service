...
Статус этого раздела — в реализации; production и аппаратная приёмка требуют
отдельного подтверждения, а не считаются выполненными по наличию кода.

Состояние реализации на 2026-09-14: код транзита, admin reroute, отдельного
sync перемещений и общего сканера подготовлен в clean worktree от `a666eaf`.
...
Полный pytest перед публикацией ещё выполняется; статуса PASS для него нет.

Read-only dry-run окна 14 дней: 4 687 документов, 10 страниц по 500, 1 608
готовых к созданию единиц, 3 079 пропущенных, 3,24 секунды. Выявлена одна
отсутствующая складская ссылка: импортируется как неактивная справочная запись,
без изменения прав и pilot allowlist. Синхронизация, флаги и release в production
на момент этой записи не включались. Реальные передачи/приёмки не выполнялись.
