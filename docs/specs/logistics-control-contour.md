...
related_code:
  ...
  - app/services/logistics_routing.py
  - app/services/logistics_transfer_sync.py
  - tasks/sync_logistics_transfers_from_onec.py
  - infra/cron/logistics_transfer_sync.sh
  - ui/src/components/logisticsCamera.ts
  - ui/src/components/useLogisticsScanQueue.ts
...
related_tests:
  ...
  - tests/test_logistics_transit.py
  - tests/test_logistics_transfer_sync.py
...
Статус кода: реализован в отдельном clean worktree. Работа начата от
подтверждённого production `5cf426e`, затем синхронизирована с параллельно
опубликованным `cda49e9`; его изменения сохранены. Это не отметка о включении
флагов: публикация подтверждается active manifest и операционным readback.
Обратная связь и приёмка команды — в [задаче №225](https://crm.master-mobile.ru/company/personal/user/115204/tasks/task/view/225/).

Read-only сверка 2026-09-12: полная выгрузка 93 активных сотрудников занимает
около 4 секунд; точному правилу соответствует Байрамов Эльвин Эйваз (`130755`).
Расчёт переключения на изолированной копии справочника: создать одну запись,
деактивировать три legacy-записи, не удалять ни одну. Доступа `timeman` у
сервисного подключения нет, поэтому смена честно отображается как неизвестная.
...
# Выпуск по результатам тестирования №225 (2026-09-14)

Утверждён отдельный выпуск: транзит, синхронизация перемещений, камера,
внешний 2D-сканер в режиме клавиатуры и согласованность счётчиков.
Незавершённая ветка внешних отправок СДЭК/Почты/маршрутки сохраняется отдельно.
Статус этого раздела — опубликован 2026-09-14; аппаратная приёмка командой
остаётся открытой и не считается выполненной по наличию кода или CI.

Состояние реализации на 2026-09-14: код транзита, admin reroute, отдельного
sync перемещений и общего сканера опубликован штатным контроллером из чистого
commit `fd10958`, подготовленного от `a666eaf`, после merge
[PR №196](https://github.com/mastermobile-inc/pricing-service/pull/196).
Release: `logistics-transit-scanner-20260914-fd10958`. Предыдущий:
`order-prepay72-20260913-a666eaf`. Актуальный общий production по-прежнему
проверяется через release-controller, а не определяется исторической записью.
Адресные backend-проверки проходят; UI — 238 тестов (29 файлов), production build,
Ruff, Black и OpenAPI check — без ошибок. Локальный Chromium smoke проверил
мобильный размер 390×844 и desktop 1280×900, переключение ЦС, очередь из 20 вводов,
admin reroute: нет белого экрана, горизонтального выхода и ошибок JavaScript.
Это тестовые данные в браузере, не аппаратная приёмка iPhone/Android/2D-сканера.
Полный [CI](https://github.com/mastermobile-inc/pricing-service/actions/runs/34809901175)
успешен: 4119 passed, 17 skipped, 5805 warnings, 26 минут 19 секунд.
Дублирующий локальный прогон остановлен после успешного CI: 1809 passed,
20 skipped до остановки; это не отдельный полный локальный PASS.
`npm audit --omit=dev`: 0 уязвимостей; предупреждения dev-зависимостей
не устранялись расширением этого выпуска.

Read-only dry-run окна 14 дней из release: 4695 документов, 10 страниц по 500,
1614 готовых к созданию единиц, 3081 пропущенный, 5,27 секунды. Первый apply:
4696 прочитано, 1614 создано, 3082 пропущено, 5 unknown_qr закрыто, 8,08 секунды.
Одна отсутствующая складская ссылка импортирована как неактивная справочная
запись без изменения прав и pilot allowlist. Все пять проблемных перемещений
найдены локально; физических событий по ним нет. Разница в количестве прочитанных
документов между dry-run и apply отражает обновление источника во время работы.

В production раздельно включены `LOGISTICS_TRANSFER_SYNC_ENABLED` и
`LOGISTICS_TRANSIT_ROUTING_ENABLED`; cron установлен в
`/etc/cron.d/pricing-logistics-transfer-sync`. Московские источники настроены
отдельным списком, региональные склады и виртуальный «Сайт» в него не включены.
SMS остаются выключены. Runtime и новые UI-ассеты отвечают HTTP 200;
контроллер подтвердил запуск и сохранение API. При перезапусках был краткий 502,
после готовности сервиса повторная проверка успешна.
Три последовательных запуска cron подтверждены в 09:06, 09:07 и 09:08 MSK:
5,705 / 5,732 / 5,696 секунды; создано 0 / 0 / 1 единиц. Проверка уникальной
пары `(source_document_type, external_id)` нашла 0 дублей. Оба marker свежие;
watchdog синхронизации и stage-outbox успешны, задержанных CRM-событий нет.
Реальные передачи/приёмки, включая изменение маршрута проблемной РТУ, выполняет
команда; агент их не выполнял. iPhone, Android и настоящий 2D-сканер не проверены.

Операционное уточнение отката: установленный release-controller v3 проверяет
`candidate.required_base_commit == active.source_commit`, поэтому прямой `switch`
на старый каталог `a666eaf` после этого выпуска будет отклонён. Быстрый откат —
остановить новый cron и выключить новые флаги с перезапуском сервиса. Если нужен
возврат прежнего кода, штатный путь — отдельный чистый revert-release поверх
текущей активной базы; не править manifest и не обходить контроллер. История и
импортированные документы сохраняются в обоих случаях.

- Конечный магазин остаётся из 1С. Отправитель выбирает для отдельных документов
  текущего рейса «Напрямую» либо «Через ЦС»; новые операции по умолчанию прямые.
- ЦС — существующий «Транзит для Москвы», не «Новый центральный». Направление
  через него доступно только для настроенных московских складов пилота.
- Приёмка на ЦС возвращает документ в очередь отправления следующим рейсом,
  не подтверждает выдачу клиенту, не запускает хранение/SMS/PICKUP_WAITING.
- Изменение точки сдачи уже в пути доступно только admin, с причиной,
  проверкой версии и отдельным событием аудита. Приёмка подтверждается отдельно.
- ОТМЕНЕНО ДЛЯ НОВОГО UX (2026-09-14): автоматическое направление из документа
  запрещает любое промежуточное плечо. Запрет редактировать конечный магазин
  сохраняется; точка текущего рейса — отдельное понятие. Legacy-клиенты совместимы.
- Готовые перемещения читаются из 1С каждую минуту, с 14-дневным окном,
  пагинацией, flock, dry-run по умолчанию и независимым marker свежести 180 секунд.
  Gate: проведён, не удалён, распечатан, история «Отсканирован», учётный
  получатель — транзит, конечный склад — из подразделения. Скан не обращается в 1С.
- Камера и аппаратный 2D-сканер используют тот же поиск. Упаковку не требуется
  снимать по регламенту; сильные блики/перекрытие QR не маскируются обещанием
  гарантированного распознавания. Bluetooth сопрягается средствами устройства.
- Проверка: iPhone/Android по 10 сканов без упаковки и через обычную плёнку,
  цель 9/10 за 3 секунды без перекрывающего блика; аппаратный сканер — 20 сканов.
  Результаты и модель устройства фиксируются отдельно, тест клавиатуры не равен
  аппаратной приёмке. Реальные handoff/receipt выполняет команда.
- Откат выключает новые флаги/cron и при необходимости возвращает immutable
  release. Документы, аудит, промежуточные и конечные физические события сохраняются.

# Changelog

- 2026-09-14 — опубликован `fd10958` (PR №196): CI 4119 passed / 17 skipped,
  UI 238 passed, включены транзит и transfer sync; подтверждены три минутных
  запуска и отсутствие дублей. Аппаратная приёмка остаётся открытой;
  зафиксировано ограничение прямого rollback в release-controller v3.
- 2026-09-14 — утверждён отдельный выпуск №225: выбор текущего плеча через
  «Транзит для Москвы», административная смена точки сдачи в пути, минутная
  загрузка перемещений и улучшения камеры/2D-сканера. Статус — в реализации.
...
- 2026-09-12 — рабочая база синхронизирована с production `cda49e9`; первый
  общий прогон на `5cf426e` остановлен при смене базы (690 passed, 11 skipped,
  два устаревших теста ассортимента уже исправлены новым production). Итоговый
  прогон выполняется на совмещённом коде, временные БД — в отдельном tmpfs.
