...
| --- | --- |
| Классификация ассортимента для витрины | Ежедневно 00:12 |
...
На момент этой сверки расписание не изменено. Тяжёлый пересчёт днём не запускается.
Запись заказов и свойств в 1С этим не включается.

### Согласованное изменение — 2026-09-10, ещё не установлено
...
### Подготовленная первая очередь — 2026-09-10
...
Устанавливать cron только после успешного release, с копией старого файла и
readback. До этого таблица фактического расписания выше остаётся верной.
Первый успешный штатный цикл должен быть проверен отдельно; дневной preflight
его не подтверждает.
